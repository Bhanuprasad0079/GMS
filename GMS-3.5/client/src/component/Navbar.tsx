...
export default function Navbar() {
  ...
  useEffect(() => {
    // 1. AUTH CHECK - Using the new apiFetch utility
    const checkLoginStatus = async () => {
      try {
    ...
    // Listen for storage changes (handles logout/login in other tabs)
    window.addEventListener("storage", checkLoginStatus);
    return () => window.removeEventListener("storage", checkLoginStatus);
  }, [pathname]);
  ...
  const handleLogout = async () => {
    try {
      // Step 1: Tell backend to clear the cookie
      await apiFetch("/api/Auth/logout", { method: "POST" });
    } catch (e) { 
      console.error("Logout error", e); 
    } finally {
      // Step 2: Clean up LOCAL data (Important: remove the token too!)
  ...
  return (
    <nav className="sticky top-0 z-50 bg-slate-900/90 dark:bg-black/90 backdrop-blur-md border-b border-slate-800 shadow-lg transition-colors duration-300 font-sans">
          ...
          <div className="hidden md:flex items-center gap-5">
            ...
            {isLoggedIn ? (
              ...
              <div className="flex items-center gap-4 animate-in fade-in slide-in-from-right duration-500">
                ...
                <Link href="/signup" className="px-5 py-2.5 text-sm font-bold text-white bg-emerald-600 hover:bg-emerald-700 rounded-lg shadow-lg transition-all hover:-translate-y-0.5">
                  Get Started
                </Link>
              </div>
            )}
      ...
      <div className={`md:hidden overflow-hidden transition-all duration-300 ease-in-out ${isOpen ? "max-h-96 opacity-100" : "max-h-0 opacity-0"}`}>
        <div className="bg-slate-900 border-t border-slate-800 px-4 py-4 space-y-2">
          <Link href="/" onClick={() => setIsOpen(false)} className={`block px-4 py-3 rounded-lg text-sm font-medium ${pathname === "/" ? "bg-slate-800 text-emerald-400" : "text-slate-300"}`}>Home</Link>
          <Link href="/about" onClick={() => setIsOpen(false)} className={`block px-4 py-3 rounded-lg text-sm font-medium ${pathname === "/about" ? "bg-slate-800 text-emerald-400" : "text-slate-300"}`}>About</Link>
          
          {isLoggedIn && (
            <Link href={dashboardLink} onClick={() => setIsOpen(false)} className={`block px-4 py-3 rounded-lg text-sm font-medium ${pathname === dashboardLink ? "bg-slate-800 text-emerald-400" : "text-slate-300"}`}>
              Dashboard
            </Link>
          )}
          ...
          <div className="border-t border-slate-800 pt-4 mt-2">
            {isLoggedIn ? (
              <button onClick={handleLogout} className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-lg bg-rose-900/30 text-rose-400 font-bold">
                <LogOut className="w-4 h-4" /> Sign Out
              </button>
            ) : (
              <div className="grid grid-cols-2 gap-3">
                <Link href="/login" onClick={() => setIsOpen(false)} className="text-center px-4 py-3 rounded-lg border border-slate-700 text-slate-300 font-bold">Login</Link>
                <Link href="/signup" onClick={() => setIsOpen(false)} className="text-center px-4 py-3 rounded-lg bg-emerald-600 text-white font-bold">Register</Link>
              </div>
            )}
          </div>
        </div>
