...
import { 
  ...
  Lock, KeyRound, Check, ChevronDown, ChevronUp, Clock, Activity
} from "lucide-react";
...
export default function AdminDashboard() {
  ...
  const fetchData = async () => {
    setIsLoading(true);
    try {
        // Robust Fetching: Each part tries to load independently
        try {
  ...
  const deleteTicket = async (id: number) => {
    ...
    await fetch(`http://localhost:5087/api/Ticket/${id}`, { method: "DELETE", credentials: "include" });
    fetchData();
  };

  const deleteUser = async (email: string) => {
    if (!confirm(`Delete user ${email}?`)) return;
    await fetch(`http://localhost:5087/api/Auth/delete-user/${email}`, { method: "DELETE", credentials: "include" });
    fetchData();
  };
